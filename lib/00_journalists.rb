...
def number_journalist
	handle_Twitter = stock
	puts "Combien y a-t-il de journalistes dans ce array?"
	puts "Il y a #{handle_Twitter.length} journalistes."
	puts "\n \n"
end
...
def number_Jnumber
	...
	number = 0
	for  x in handle_Twitter
		number += 1 if x =~ /[1234567890]/
	end
...
def number_aude
	...
	number = 0
	for  x in handle_Twitter
		number += 1 if x.downcase =~ /aude/
	end
	puts "Il y a #{number} handles qui contiennent les 4 lettres du prénom 'Aude' à la suite."
	puts "\n \n"
end

# => Question 4:
def number_majuscule
	handle_Twitter = stock
	puts "Combien commencent par une majuscules (première lettre juste après le @)?"
	number = 0
	for  x in handle_Twitter
		number += 1 if x =~ /@\p{Upper}/
	end
	puts "Il y a #{number} handles qui commencent par une majuscule après le @."
	puts "\n \n"
end

# => Question 5:
def number_just1_maj
	handle_Twitter = stock
	...
	puts "Il y a #{number} handles qui contiennent au moins une majuscule."
	puts "\n \n"
end
...
def number_underscore
	...
	handle_Twitter.grep(/_/).count
	puts "Il y a #{handle_Twitter.grep(/_/).count}  underscores dans tous les pseudos."
	puts "\n \n"
end
...
def  length_handle
	...
	puts "Les 50 handles les plus courts sont: #{handle_Twitter.min(50) {|xa, xb| xa.length <=> xb.length}}"
	puts "\n \n"
end
...
def position_array
	...
	puts "@epenser est en position #{handle_Twitter.index("@epenser")}."
	puts "\n \n"
end
